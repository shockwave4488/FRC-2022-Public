...
import frc.robot.commands.drive.DriveAndFaceAprilTag;
...
public class CandyRobotContainer extends BaseRobotContainer {
  ...
  protected void configureButtonBindings() {
    ...
    enabled.onTrue(new RunCommand(() -> field.setRobotPose(swerve.getOdometry())));
    enabled.onTrue(
        new RunCommand(
            () -> {
              for (AprilTag tag :
    ...
    SmartDashboard.putNumber("Autonomous max speed", 2.0);
    TrajectoryConfig config =
        new TrajectoryConfig(
                SmartDashboard.getNumber("Autonomous max speed", 2.0),
                DriveTrainConstants.SWERVE_DRIVE_MAX_ACCEL)
            .setKinematics(swerve.getKinematics());

    CommandBase cubeAprilTagCommand = createAprilTagCommand(visibleCubeTargets, config, field);
    CommandBase stationAprilTagCommand =
        createAprilTagCommand(visibleStationTargets, config, field);

    driverJoystick.leftBumper().whileTrue(cubeAprilTagCommand);
    driverJoystick.rightBumper().whileTrue(stationAprilTagCommand);

    SmartDashboard.putData(
        "Limelight Snapshot", new InstantCommand(camera::takeSnapshot).withName("Take snapshot"));
  }

  private CommandBase createAprilTagCommand(
      Supplier<List<AprilTagPhotonTarget>> visibleTargets, TrajectoryConfig config, Field2d field) {
    DriveAndFaceAprilTag aprilTagCommand =
    ...
    return new ConditionalCommand(
        new InstantCommand(
            ...
            .andThen(
                aprilTagCommand.alongWith(
                    new InstantCommand(
                        () ->
                            field
                                .getObject("Trajectory")
                                .setTrajectory(aprilTagCommand.getTrajectory())))),
        new InstantCommand(),
